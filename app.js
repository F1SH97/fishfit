...
(function () {
  ...
  function goalHero(clickable){
    ...
    return '<div class="pcard'+(clickable?' lift':'')+'"'+(clickable?' data-action="goal"':'')+' style="padding:20px;display:grid;grid-template-columns:1.1fr 1.4fr;gap:22px'+(clickable?';cursor:pointer':'')+'">'
  ...
  function runningView(){
    ...
    const table='<div class="pcard" style="padding:4px"><table><thead><tr>'+["Date","Session","Dist","Pace","Avg HR","Note"].map(h=>'<th>'+h+'</th>').join("")+'</tr></thead><tbody>'
  ...
  function shell(){
    const sidebar='<aside class="sidebar"><div style="display:flex;align-items:center;gap:10px;padding:4px 6px 22px"><div style="width:30px;height:30px;border-radius:9px;background:linear-gradient(135deg,var(--warm),var(--coral));display:grid;place-items:center;color:#0E1420">'+ic("sunrise",17)+'</div><div><div style="font-weight:700;font-size:14.5px;color:var(--text);letter-spacing:-0.2px">PerformanceOS</div><div style="font-size:10px;color:var(--muted)">personal \u00b7 Garmin-first</div></div></div>'
      +NAV.map(n=>'<button class="nav'+(state.view===n[0]?' navOn':'')+'" data-action="view" data-view="'+n[0]+'">'+ic(n[1],17)+' '+n[0]+'</button>').join("")
      +'<div style="margin-top:auto;padding-top:16px"><div class="pcard" style="padding:12px;background:var(--panel2)"><div style="display:flex;align-items:center;gap:7px;font-size:11.5px;color:var(--dim);font-weight:600">'+ic("radio",13)+' Data source</div><div style="font-size:11px;color:var(--muted);margin:6px 0 9px;line-height:1.4">Fixtures modeled on Garmin MCP tool outputs.</div><button class="connectbtn">'+ic("zap",12)+' Connect Garmin MCP</button></div></div></aside>';
